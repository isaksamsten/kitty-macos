...
static void
convert_from_python_macos_autohide_buttons(PyObject *val, Options *opts) {
    opts->macos_autohide_buttons = PyObject_IsTrue(val);
}

static void
convert_from_opts_macos_autohide_buttons(PyObject *py_opts, Options *opts) {
    PyObject *ret = PyObject_GetAttrString(py_opts, "macos_autohide_buttons");
    if (ret == NULL) return;
    convert_from_python_macos_autohide_buttons(ret, opts);
    Py_DECREF(ret);
}
...
convert_opts_from_python_opts(PyObject *py_opts, Options *opts) {
    ...
    if (PyErr_Occurred()) return false;
    convert_from_opts_macos_autohide_buttons(py_opts, opts);
    if (PyErr_Occurred()) return false;
